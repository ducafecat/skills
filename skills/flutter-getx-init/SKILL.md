...
- Home 右上角组件样式入口，以及通过底部菜单切换 theme / i18n 的两个按钮
...
- 组件调试页使用 `assets/lib/pages/component_styles/`，路由、翻译键和共享主题菜单见源码模板
...
```sh
flutter pub add get go_router dio native_dio_adapter freezed_annotation json_annotation shared_preferences logger adaptive_theme:3.7.2 flutter_cache_manager cached_network_image flutter_svg
```

`adaptive_theme` 固定为已验证的 `3.7.2`；升级时先核对其 ThemeData 与 Flutter Material 的兼容性，再调整模板。
...
基础脚手架以 `references/file-templates.md` 为准；`lib/common/widgets/**` 以 `assets/lib/common/widgets/` 为唯一源码模板。同时从 `assets/lib/pages/component_styles/` 递归复制或合并组件调试页及其入口。递归复制或合并全部组件，同时落地模板中的样式 token、三语文案和入口导出。组件使用空感知集合元素等语法，确认目标 Dart SDK 支持后再落地。

必出目录：

- `lib/common/**`（13 个子目录 + `index.dart`）
- `lib/pages/splash|welcome|login|home|component_styles`
...
- 业务页面状态用 `GetxController` + `GetBuilder` + `update([id])`；组件调试页的纯本地演示状态保留资源中的 StatefulWidget 实现，dispose 释放控制器
...
- Home 右上角新增组件样式按钮，使用 `context.push(AppRoutes.componentStyles)` 打开独立 ComponentStylesPage；同步路由、页面导出、三语标题和共享 `showThemeModeSheet`。
- Home 另外两个按钮分别使用 `showModalBottomSheet`：主题提供浅色 / 深色 / 跟随系统，语言提供 en / zh-CN / zh-TW。主题使用 AdaptiveTheme，语言使用 LocaleService 持久化；底部菜单用自身 context 的 Navigator 关闭，禁止 `Get.back`。
...
- 模板文档中的入口应为 Home 右上角 → 组件样式，路由为 `/component-styles`，不要照搬业务项目的“我的”页面入口。
...
没有测试或默认 widget 测试对不上脚手架时，按模板更新 `test/widget_test.dart`，或写明为什么没跑。同步生成 `test/component_styles_navigation_test.dart`，验证真实 Home 入口、共享主题菜单和返回；包名占位符同样替换。
...
- `lib/pages/component_styles/` 已落地，Home 右上角入口能打开、返回，主题菜单可用
...
- 应用能走 Splash（0.5 秒）→ Welcome 三页 / Login（预填演示账号）→ Home（右上角进入组件样式、底部菜单切主题/语言），或剩余阻塞已写明
